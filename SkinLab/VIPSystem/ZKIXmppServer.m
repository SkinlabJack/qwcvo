...
//	[xmppCapabilities           activate:xmppStream];
	...
	[xmppStream setHostName:@"tryseason.com"];
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
	...
	if ([message isChatMessageWithBody]) {
        XMPPUserCoreDataStorageObject *user = [xmppRosterStorage userForJID:[message to]
        ...
        if ([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive) {
            ...
            NSDictionary *dic = @{@"Message": body, @"From": from, @"To": to};
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ReceiveNewMessage" object:self userInfo:dic];
...
- (NSArray *)getMessageData:(NSString *)fromName selfName:(NSString *)selfName{
    ...
    for (XMPPMessageArchiving_Message_CoreDataObject *object in array) {
        DLog(@"%@", object.messageStr)
        DLog(@"%@", object.bareJidStr)
        DLog(@"%@", object.isOutgoing?@"yes": @"no")
        
        if ([object.bareJid.user isEqualToString:fromName] || [object.bareJid.user isEqualToString:selfName]) {
            if (object.isOutgoing) {
                NSDictionary *dic = @{@"Message": object.body, @"From": selfName, @"To": object.bareJid.user};
                [messagesArray addObject:dic];
            }else{
                NSDictionary *dic = @{@"Message": object.body, @"From": object.bareJid.user, @"To": selfName};
                [messagesArray addObject:dic];
            }
